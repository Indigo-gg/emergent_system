...
3. **力计算**（拆分为两个独立分量，见下文详解）：
   - **F_particle**：粒子-粒子相互作用力（由 GEP 势能 U 经符号求导获得）
   - **F_env**：环境梯度感知力（由 GEP 化学趋向公式驱动）
   - `F_total = F_particle + F_env`
4. **粘性阻尼**：`F_total -= γ * v`（防止数值振荡）
...
> **问题**：v5 的 GEP 势能公式 U(dist, density, ...) 只依赖粒子间的相对量，天然满足 F_ij = -F_ji（动量守恒）。
> v6 引入环境变量 `nutrient` 和 `waste` 后，如果直接加入势能公式，会产生**非对称力**：
> - 粒子 i 受力：F_ij = -dU/dr|_{nutrient=N_i}
> - 粒子 j 受力：F_ji = -dU/dr|_{nutrient=N_j}
> - 因为 N_i ≠ N_j，所以 F_ij ≠ -F_ji → 动量守恒被破坏 → 粒子团凭空加速飞出屏幕
>
> **解决**：将力拆分为两个独立分量，各自保证物理正确性。

**分量 A：粒子-粒子相互作用力 F_particle（保证动量守恒）**

GEP 势能公式 U 的终端集**不包含** `nutrient` 和 `waste`。只使用对称变量：
- `dist`：两粒子间距离（对称）
- `density`：局部密度（对称）
- `avg_nutrient`：两粒子中点的营养浓度 `(nutrient_i + nutrient_j) / 2`（对称）
- `avg_waste`：两粒子中点的废弃物浓度（对称）
- `speed`、`angle`、`state[0..3]`、`neighbor_count`：粒子自身属性

```python
# GPU 端：计算粒子 i 受粒子 j 的力
avg_nut = 0.5 * (nutrient_at(pos_i) + nutrient_at(pos_j))
avg_wst = 0.5 * (waste_at(pos_i) + waste_at(pos_j))
# avg_nut 和 avg_wst 对 i,j 对称 → F_ij = -F_ji → 动量守恒
F_ij = -vm_execute(udr_bytecode, constants, dist, density, avg_nut, avg_wst, ...)
```

**分量 B：环境梯度感知力 F_env（趋化性，独立于粒子交互）**

> **问题**：粒子-粒子力只在有邻居时才产生。如果一个粒子周围没有其他粒子，它就无法感知环境梯度——它是"瞎子"。
>
> **解决**：为每个粒子计算其所在位置的营养场和废弃物场的**空间梯度**，由 GEP 单独进化一个趋化性公式。

```python
# 每步预计算：环境场的空间梯度（Sobel 算子或中心差分）
@ti.kernel
def compute_env_gradients():
    for i, j in nutrient_field:
        grad_nut_x[i,j] = (nutrient_field[i+1,j] - nutrient_field[i-1,j]) / (2 * cell_size)
        grad_nut_y[i,j] = (nutrient_field[i,j+1] - nutrient_field[i,j-1]) / (2 * cell_size)
        grad_wst_x[i,j] = (waste_field[i+1,j] - waste_field[i-1,j]) / (2 * cell_size)
        grad_wst_y[i,j] = (waste_field[i,j+1] - waste_field[i,j-1]) / (2 * cell_size)

# 每个粒子的环境感知力（不需要邻居，独立计算）
@ti.func
def compute_env_force(i, grad_nut, grad_wst):
    # GEP 进化出的趋化性公式
    # 输入：grad_nut_x, grad_nut_y, grad_wst_x, grad_wst_y, nutrient, waste, speed
    # 输出：F_env_x, F_env_y
    # 例如：F_env = k1 * ∇nutrient - k2 * ∇waste
    F_env_x = vm_execute(chemotaxis_bytecode, constants,
                         grad_nut.x, grad_nut.y, grad_wst.x, grad_wst.y,
                         nutrient_at(pos[i]), waste_at(pos[i]), speed[i])
    F_env_y = vm_execute(chemotaxis_bytecode, constants, ...)
    return F_env_x, F_env_y
```
...
@dataclass
class GEPGenome:
    # 分量 A：粒子-粒子势能 U（终端集：dist, density, avg_nutrient, avg_waste, speed, angle, state[0..3], neighbor_count）
    potential_expr: str
    # 分量 B：趋化性公式（终端集：grad_nut_x, grad_nut_y, grad_wst_x, grad_wst_y, nutrient, waste, speed）
    chemotaxis_expr: str
    # 状态更新规则
    state_expr: str
    # 感知规则
...
**两个公式独立进化，独立编译为字节码，各自有独立的符号求导链。**

#### 空间哈希溢出的硬碰撞保护（v6 关键修正）

> **问题**：桶容量硬上限 128，超出粒子被拒绝插入 → 变成"幽灵" → 无视物理体积重叠 → dist=0 → NaN/Inf。
>
> **解决**：不能简单丢弃溢出粒子。在积分步增加**全局底层硬碰撞**，不经过 GEP 进化，不可被绕过。

```python
HARD_REPULSION_EPSILON = 0.01  # 最小安全距离（物理半径）
HARD_REPULSION_STRENGTH = 100.0  # 硬斥力强度（远大于任何 GEP 公式）

@ti.func
def hard_collision_check(i, j, dist_ij):
    """底层硬碰撞：防止粒子无限重叠，不经过 GEP，不可被绕过"""
    if dist_ij < HARD_REPULSION_EPSILON and dist_ij > 0:
        # 施加不可进化的强斥力
        dx = pos_x[i] - pos_x[j]
        dy = pos_y[i] - pos_y[j]
        overlap = HARD_REPULSION_EPSILON - dist_ij
        force_mag = HARD_REPULSION_STRENGTH * overlap
        fx = force_mag * dx / dist_ij
        fy = force_mag * dy / dist_ij
        force_x[i] += fx
        force_y[i] += fy
        # 对 j 的反作用力在 j 的循环中处理（或在此处原子减）

@ti.func
def integrate(i, dt: ti.f32):
    # ... 正常积分 ...
    # 积分后：检查是否与其他粒子重叠（桶溢出的幽灵粒子会走到这里）
    # 通过全局邻居扫描（或扩大搜索半径）检测重叠
    # 检测到 dist < epsilon → 强制推开
```
...
| 风险 | 硬件原因 | 应对 |
|------|----------|------|
| **寄存器溢出（Register Spilling）** | `stack = ti.Vector([0.0]*16)` 若用动态索引访问，LLVM 无法放入寄存器，溢出到本地内存（延迟 100x） | Phase 1 必须用 Taichi profiler 检查 kernel，确认 `stack` 被寄存器化。若溢出，**果断放弃栈式 VM**，改用方案 A 或 B（见下文） |
...
**VM 寄存器溢出的明确回退方案（v6 强化）**：

如果 Phase 1 压力测试发现 `stack` 溢出到 Local Memory（性能下降 > 10x），**不要试图优化栈式 VM，直接切换方案**：

**方案 A（推荐）：基于固定寄存器的 VM**
```python
# 用 4 个独立的 ti.f32 变量代替 ti.Vector 动态索引
# LLVM 可以将独立变量放入寄存器，不会溢出
@ti.func
def vm_execute_fixed_reg(bytecode, constants, var0, var1, ...):
    reg0, reg1, reg2, reg3 = 0.0, 0.0, 0.0, 0.0
    # 操作码硬编码为 if-elif 链，直接操作寄存器
    # 无动态数组索引 → 无寄存器溢出
```

**方案 B：exec() 动态编译（彻底放弃 VM）**
```python
# 每代 10 个基因组，10 次 Taichi JIT 编译
# Taichi 缓存命中时编译极快（~0.1-0.2s），完全可接受
def compile_to_taichi_kernel(gep_tree: Node) -> str:
    """将 GEP 表达式树翻译为 Taichi Python 代码字符串"""
    code = """
@ti.kernel
def compute_forces_custom(pos_x: ti.template(), ...) -> ti.f32:
    for i in range(n_particles):
        # ... 展开的表达式，无函数调用开销 ...
"""
    return code
...
# 动态生成并执行
exec(compile_to_taichi_kernel(genome.potential_tree), namespace)
custom_kernel = namespace['compute_forces_custom']
# 性能：纯硬编码 Taichi kernel，比 VM 快 10-50x
```

**方案 B 的关键优势**：
- 每代 10 个公式 × 0.2s 编译 = 2s 开销 vs GPU 跑 50000 步的几十秒 → 可忽略
- Taichi 有 JIT 缓存，相同公式不会重复编译
- 性能 = 原生硬编码 kernel，无 VM 解释开销
- 天然避免寄存器溢出问题
...
# 4. 如果 VM 性能不达标 → 立即切换方案 B（exec 动态编译）
```
...
v6 新增：GEP 有两套独立的公式，各自有独立的终端集：

**公式 A — 势能 U（粒子-粒子交互，保证动量守恒）**：
- 终端集：`{dist, density, avg_nutrient, avg_waste, speed, angle, state[0..3], neighbor_count}`
- `avg_nutrient` = 两粒子中点的营养浓度（对称量，保证 F_ij = -F_ji）
- `avg_waste` = 两粒子中点的废弃物浓度（对称量）
- 不直接使用单点的 `nutrient`/`waste`，破坏对称性
- 示例：`U = 1.5 * dist - sin(avg_nutrient * 3.14) * density`

**公式 B — 趋化性 F_env（环境梯度感知，独立于粒子交互）**：
- 终端集：`{grad_nut_x, grad_nut_y, grad_wst_x, grad_wst_y, nutrient, waste, speed}`
- 不依赖其他粒子，每个粒子独立计算
- 示例：`F_env = 2.0 * grad_nut_x - 0.5 * grad_wst_x`（沿营养梯度游动，避开废弃物梯度）

**GEP 基因组结构**：
```python
@dataclass
class GEPGenome:
    # 分量 A：粒子-粒子势能 U（力由 F = -∇U 自动获得，保证动量守恒）
    # 终端变量：dist, density, avg_nutrient, avg_waste, speed, angle, state[0..3], neighbor_count
    # 注意：使用 avg_nutrient（对称量）而非单点 nutrient，保证 F_ij = -F_ji
    potential_expr: str      # 例如: "1.5 * dist - sin(avg_nutrient * 3.14) * density"
    # 分量 B：趋化性公式（环境梯度感知力，独立于粒子交互）
    # 终端变量：grad_nut_x, grad_nut_y, grad_wst_x, grad_wst_y, nutrient, waste, speed
    chemotaxis_expr: str     # 例如: "2.0 * grad_nut_x - 0.5 * grad_wst_x"
    # 状态更新规则：内部状态的演化
...
- 终端集 A（势能 U）：`{dist, density, avg_nutrient, avg_waste, speed, angle, state[0..3], neighbor_count}`（12 个变量，`avg_nutrient`/`avg_waste` 为对称量）
- 终端集 B（趋化性 F_env）：`{grad_nut_x, grad_nut_y, grad_wst_x, grad_wst_y, nutrient, waste, speed}`（7 个变量，环境梯度 + 本地浓度）
...
fitness_env = w1 * total_energy_harvested     # 粒子从环境中获取的总能量
            + w2 * survival_rate_integral     # 存活率的时间积分（非瞬时值，见下文）
...
> **问题**：如果粒子完全不移动，当营养热点飘过时疯狂吸收，热点走了就休眠。`survival_rate` 极高，`move_cost` 为 0，适应度很高。进化必走"捷径"——产出一堆不动的"石头物种"。
>
> **解决**：严惩不作为。

```
# 反蹲坑：平均速度过低 → 直接判 0
avg_speed = mean(|v|) over all particles and timesteps
if avg_speed < 0.1:
    fitness = 0.0  # 不是好策略，是蹲坑

# 或更柔和：速度因子
speed_factor = clamp(avg_speed / 0.5, 0.0, 1.0)  # 速度 < 0.5 时按比例扣分
fitness *= speed_factor
```

**存活率积分修正（v6 关键修正）**：

> **问题**：`survival_rate` 是仿真结束时的瞬时值。一个物种展示了惊艳的"脉冲式迁徙+繁殖"涌现，但第 50000 步刚好处于种群低谷（存活率 8%），直接被判 0 分。
>
> **解决**：用"生命存续总面积"（积分）代替瞬时值。

```
# 而非：survival_rate = alive_count[t=50000] / total_particles
# 改为：
survival_integral = sum(alive_count[t] for t in 0..50000) / (50000 * total_particles)
# 值域 [0, 1]，反映整个生命周期的存续能力
# 一个活跃了 40000 步然后灭绝的物种，积分 > 一个全程静止存活的物种
```
...
- 反蹲坑机制确保"能活下来"必须通过**主动觅食**实现，而非消极等待
...
> **问题**：时序特征在整个 `steps_per_eval` 期间计算。如果一个物种活跃了 500 步然后全体休眠 49500 步，活跃特征被巨大的休眠零值"稀释"。两个活跃期行为完全不同的物种，可能因为都有 90% 时间在休眠，导致 15D 不变量极其相似，Novelty Archive 区分度崩溃。
>
> **解决**：时序特征**只统计活跃粒子**，或按"有效活跃时间"加权。

```python
def extract_features(snapshots: list[SampleSnapshot]):
    """只统计活跃粒子的特征，忽略休眠粒子"""
    active_snapshots = []
    for snap in snapshots:
        # 只保留活跃粒子（energy > 0）的统计量
        active_mask = snap.energies > 0
        if active_mask.sum() < 10:  # 活跃粒子太少，跳过此帧
            continue
        active_snapshots.append(snap.filter(active_mask))

    if len(active_snapshots) < 10:  # 活跃帧太少，整个基因组被判为死寂
        return None  # 触发死寂过滤

    # 在活跃帧上计算 15 维不变量
    return compute_15d_features(active_snapshots)
```
...
- ~~如果 `survival_rate < 0.1`（90% 粒子死亡）→ fitness = 0~~ → v6 修正：如果 `survival_integral < 0.1`（整个生命周期中平均存活率 < 10%）→ fitness = 0。用积分面积代替瞬时值，避免"脉冲式涌现刚好在结束时处于低谷被误杀"
- 如果 `entropy_mean > 0.95 * max_entropy`（完全均匀分布）→ fitness = 0
- 如果 `speed_var_mean < 0.001`（所有粒子静止）→ fitness = 0
- v6 新增：如果 `avg_speed < 0.1`（蹲坑检测：粒子平均速度过低）→ fitness = 0
...
v6 修正：CPU 模式推理耗时被严重低估。3B 视觉模型在 CPU 上处理高频图像（500 帧轨迹叠加），一次推理至少 1-3 分钟。

- **方案 A（推荐）：GPU 时分复用 + keep_alive:0**
  ```python
  # 1. 暂存 Taichi 仿真状态（粒子位置、速度等）到 CPU
  # 2. ti.reset() 释放 GPU 显存
  # 3. 调用 Ollama（GPU 模式，秒出结果）
  #    resp = requests.post("...", json={"model": "qwen2.5vl:3b", "keep_alive": 0, ...})
  # 4. Ollama 自动卸载模型（keep_alive:0 立即释放显存）
  # 5. ti.init(arch=ti.cuda) 恢复 Taichi
  # 6. 从 CPU 恢复仿真状态
  # 每次 VLM 调用总开销：~10-20 秒（含状态迁移），远快于 CPU 的 1-3 分钟
  ```
- **方案 B：Ollama CPU 模式**（`OLLAMA_NUM_GPU=0`）
  - 每次 1-3 分钟，每天 50-100 次 = 1-5 小时
  - 对无人值守运行可接受，但会严重拖慢进化速度
  - 降低图片分辨率（128×128 代替 256×256）可加速 ~2x

**频率**：每天最多触发 ~50-100 次 VLM（新颖性告警）。
...
| 模块 | 预算 | 用途 |
|------|------|------|
| 粒子仿真 | 4 GB | 100 万粒子状态 + 空间哈希 + VM 栈 + 中间缓冲 + 营养/废弃物场 + 梯度场（∇nutrient, ∇waste） |
| 渲染管线 | 0.5 GB | 离屏渲染帧缓冲 |
...
### Phase 2.5：环境层（3-4 天）★ v6 新增
- [ ] **营养场**：Taichi sparse grid，扩散+衰减+注入 kernel
- [ ] **废弃物场**：Taichi sparse grid，扩散+衰减 kernel
- [ ] **移动热点**：营养注入点沿螺旋/随机路径漂移
- [ ] **环境梯度计算**：每步预计算 ∇nutrient、∇waste（Sobel/中心差分）
- [ ] **力拆分架构**：F_total = F_particle + F_env
  - F_particle：粒子-粒子势能力（GEP 终端集用对称量 avg_nutrient/avg_waste）
  - F_env：环境梯度感知力（GEP 独立进化趋化性公式）
- [ ] **GEP 双公式基因组**：potential_expr + chemotaxis_expr，各自独立编译字节码
- [ ] **全局硬碰撞**：溢出粒子的 dist < epsilon 强斥力（不可被 GEP 绕过）
- [ ] **粒子环境交互**：营养摄取、代谢消耗、废弃物沉积、休眠/苏醒/死亡
- [ ] **环境驱动适应度**：total_energy_harvested + survival_integral + energy_variance
- [ ] **反蹲坑机制**：avg_speed < 0.1 → fitness *= 0
- [ ] **休眠特征修正**：特征提取只统计活跃粒子
...
**总计：约 15-21 天**（v6 新增 Phase 2.5 环境层 3-4 天）
...
| **动量守恒被环境变量破坏** | ~~已解决~~ | — | v6 力拆分：粒子交互力用对称量（avg_nutrient），环境力独立计算（梯度感知） |
| **空间哈希黑洞 NaN** | ~~已解决~~ | — | v6 增加全局底层硬碰撞（Hard Repulsion），溢出粒子不再是"幽灵" |
| **适应度蹲坑漏洞** | ~~已解决~~ | — | v6 反蹲坑：avg_speed < 0.1 → fitness = 0 |
| **休眠孢子特征稀释** | ~~已解决~~ | — | v6 特征提取只统计活跃粒子 |
| **死寂过滤过于严苛** | ~~已解决~~ | — | v6 用存活率积分代替瞬时值 |
| **VLM CPU 耗时低估** | 中 | 进化速度拖慢 | 优先用 GPU 时分复用（keep_alive:0），CPU 模式为兜底 |
...
gep:
  # 终端集 A：势能 U 的变量（对称量保证动量守恒）
  terminal_set_potential: ["dist", "density", "avg_nutrient", "avg_waste", "speed", "angle", "state_0", "state_1", "state_2", "state_3", "neighbor_count"]
  # 终端集 B：趋化性公式的变量（环境梯度感知）
  terminal_set_chemotaxis: ["grad_nut_x", "grad_nut_y", "grad_wst_x", "grad_wst_y", "nutrient", "waste", "speed"]
...
safety:
  ...
  hard_repulsion_epsilon: 0.01  # v6: 最小安全距离（物理半径）
  hard_repulsion_strength: 100.0  # v6: 硬斥力强度（不可被 GEP 绕过）
  min_active_speed: 0.1       # v6: 反蹲坑阈值（avg_speed < 此值 → fitness=0）
