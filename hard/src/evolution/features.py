"""
12-dimensional time-invariant feature extraction.
...
class FeatureExtractor:
    """Extracts 12D time-invariant features from simulation samples."""
    ...
    def sample(self, particles, cfg: dict):
        """
        ...
        # Spatial entropy
        entropy = self._spatial_entropy(px, py, w, h)

        # Island count
        islands = self._island_count(px, py, cfg)

        # Speed variance
        speeds = np.sqrt(vx**2 + vy**2)
        speed_var = float(np.var(speeds))

        # Angular momentum (scalar: cross product of position and velocity)
        # L = r × v = x*vy - y*vx (relative to center of mass)
        cx, cy = np.mean(px), np.mean(py)
        angular_mom = np.sum((px - cx) * vy - (py - cy) * vx)

        # Density Laplacian variance
        density_lap_var = self._density_laplacian_var(px, py, w, h)
    ...
    def compute_features(self) -> np.ndarray:
        """
        Compute 12D feature vector from collected samples.
    ...
    def _fft_top3(self, time_series):
        ...
        top3 = sorted_amps[:3] / max(n, 1)  # normalize
